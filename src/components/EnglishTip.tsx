...
export default function EnglishTip() {
  return (
    <div className="space-y-16 pb-32">
      {/* HEADER */}
      <header className="border-b-2 border-white/20 pb-8 mb-12">
        ...
        <h1 className="text-5xl font-bold leading-tight mb-3 tracking-tight">
      ...
      <section className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-0 border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
          {[
            ...
            <div key={idx} className="bg-navy-800/50 p-6 text-center border-r border-white/5 last:border-r-0 relative group hover:bg-navy-800 transition-colors">
              <span className="block font-mono text-[10px] text-gray-500 mb-2 tracking-wider">TẦNG {item.step}</span>
              <span className="block text-base font-bold mb-2">{item.label}</span>
              <span className="block text-[11px] text-gray-400 leading-tight">{item.hint}</span>
              {idx < 4 && (
                <div className="hidden md:block absolute -right-2 top-1/2 -translate-y-1/2 z-10 text-white/10">
      ...
      {/* SWIFT MNEMONIC */}
      <section className="bg-navy-950 p-10 rounded-[2.5rem] border border-white/5 shadow-inner relative overflow-hidden">
        <div className="absolute top-0 right-0 p-8 opacity-5">
          <Zap className="w-32 h-32" />
        </div>
        <div className="font-mono text-[10px] tracking-[0.4em] text-white/40 mb-8 uppercase">
        ...
        <div className="space-y-6 relative z-10">
          {[
            ...
            <div key={item.l} className="flex items-baseline gap-8 group">
              <span className="font-mono text-4xl font-bold text-white/10 w-10 flex-shrink-0 group-hover:text-accent/20 transition-colors">{item.l}</span>
              <span className="font-bold text-xl w-32 flex-shrink-0">{item.w}</span>
              <span className="text-base text-white/60">{item.d}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* EXAMPLES SECTION */}
      <section className="bg-accent/5 border border-accent/20 p-10 rounded-[2.5rem] space-y-10">
        ...
        {[
          ...
          <div key={i} className="space-y-4 border-b border-accent/10 last:border-0 pb-10 last:pb-0">
            <div className="text-xl italic text-gray-400 font-serif">「 {ex.v} 」</div>
            <div className="flex flex-wrap gap-2">
              {ex.steps.map((s, j) => (
                <span key={j} className="font-mono text-[10px] px-3 py-1.5 bg-navy-900 border border-white/10 rounded-lg text-gray-400">
                  {s}
                </span>
              ))}
            </div>
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-4 border-b border-white/10 pb-4">
          <span className="bg-white text-navy-900 px-3 py-1 font-mono text-xs font-bold tracking-widest rounded">TẦNG 01</span>
          <h2 className="text-2xl font-bold">Xác định Loại Câu — 9 Kiểu</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            ...
            <div key={i} className={`glass p-6 rounded-2xl border border-white/10 border-t-4 ${item.c} space-y-3`}>
              <div className="font-mono text-[10px] text-gray-500 tracking-widest uppercase">{item.t}</div>
              <div className="text-lg font-bold">{item.n}</div>
              {item.tr && <div className="text-[11px] text-red-400 font-medium">{item.tr}</div>}
              <div className="bg-navy-950 p-3 rounded-lg font-mono text-[11px] text-blue-400 whitespace-pre-line leading-relaxed">{item.f}</div>
              <div className="text-xs italic text-gray-400">Ex: <span className="text-accent not-italic font-semibold">{item.ex}</span></div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-4 border-b border-white/10 pb-4">
          <span className="bg-white text-navy-900 px-3 py-1 font-mono text-xs font-bold tracking-widest rounded">TẦNG 02</span>
          <h2 className="text-2xl font-bold">Xác định Chủ Ngữ — Bảng Đối Chiếu</h2>
        </div>
            ...
            <tbody className="divide-y divide-white/5">
              {[
                ...
                <tr key={i} className="hover:bg-white/5 transition-colors">
                  <td className="px-6 py-4 text-gray-400 italic">{row.v}</td>
                  <td className="px-6 py-4 font-bold font-mono text-lg">{row.e}</td>
                  <td className="px-6 py-4 font-mono">{row.g}</td>
                  <td className="px-6 py-4">
                    <span className={`text-[10px] font-mono px-2 py-1 rounded-full ${row.w ? 'bg-red-500/10 text-red-400' : 'bg-green-500/10 text-green-400'}`}>
                      {row.n}
                    </span>
                  </td>
                </tr>
              ...
              <tr className="bg-yellow-500/5">
                <td colSpan={4} className="px-6 py-4 text-xs text-yellow-500 italic text-center">
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-4 border-b border-white/10 pb-4">
          <span className="bg-white text-navy-900 px-3 py-1 font-mono text-xs font-bold tracking-widest rounded">TẦNG 03</span>
          <h2 className="text-2xl font-bold">Xác định Thì — Hỏi 3 Câu & Nhìn Từ Tín Hiệu</h2>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="glass p-6 rounded-2xl border-l-4 border-l-accent space-y-4">
            <div className="font-mono text-xs text-accent font-bold uppercase tracking-wider">① Việc này xảy ra KHI NÀO?</div>
            <div className="flex flex-wrap gap-2">
              {['Đang → Hiện tại', 'Đã xong → Quá khứ', 'Sẽ → Tương lai', 'Vừa xong → Hoàn thành'].map(chip => (
                <span key={chip} className="text-[11px] px-3 py-1.5 bg-navy-900 border border-white/10 rounded-full text-gray-300">{chip}</span>
              ))}
            </div>
          </div>
          <div className="glass p-6 rounded-2xl border-l-4 border-l-accent space-y-4">
            <div className="font-mono text-xs text-accent font-bold uppercase tracking-wider">② Có đang diễn ra NGAY LÚC NÓI?</div>
            <div className="flex flex-wrap gap-2">
              {['CÓ → Thêm -ing (Progressive)', 'KHÔNG → Dùng Simple'].map(chip => (
                <span key={chip} className="text-[11px] px-3 py-1.5 bg-navy-900 border border-white/10 rounded-full text-gray-300">{chip}</span>
              ))}
            </div>
          </div>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {[
                ...
                <tr key={i} className={`hover:bg-white/5 transition-colors border-l-4 ${row.c}`}>
                  <td className="px-6 py-4">
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-4 border-b border-white/10 pb-4">
          <span className="bg-white text-navy-900 px-3 py-1 font-mono text-xs font-bold tracking-widest rounded">BỔ SUNG TẦNG 03</span>
          <h2 className="text-2xl font-bold">Xác Định Thì Theo Trạng Thái Hành Động</h2>
        </div>
        ...
        <div className="glass p-8 rounded-[2rem] space-y-8">
          <div className="font-mono text-[10px] tracking-[0.2em] text-gray-500 uppercase">// CÂY QUYẾT ĐỊNH — TRẠNG THÁI HÀNH ĐỘNG</div>
          
          <div className="bg-navy-900/50 border-l-4 border-l-blue-500 p-4 rounded-r-xl">
            <div className="font-mono text-[11px] text-blue-400 mb-1">❶ Hỏi THỜI ĐIỂM trước</div>
            <div className="text-sm">Hiện tại? &nbsp;→ tiếp tục Q2a &nbsp;|&nbsp; Quá khứ? → Q2b &nbsp;|&nbsp; Tương lai? → Q2c</div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-4">
              <div className="font-mono text-[10px] text-gray-500 tracking-widest uppercase">HIỆN TẠI → hỏi Q2a</div>
              <div className="bg-navy-900/50 border-l-4 border-l-green-500 p-4 rounded-r-xl space-y-2">
                <div className="text-xs font-bold text-green-400">Đang diễn ra ngay?</div>
                <div className="text-xs text-gray-300">CÓ → <strong>Present Continuous</strong></div>
                <div className="font-mono text-[10px] text-blue-400">S + am/is/are + V-ing</div>
              </div>
              <div className="bg-navy-900/50 border-l-4 border-l-green-700 p-4 rounded-r-xl space-y-2">
                <div className="text-xs font-bold text-green-700">Thói quen / sự thật?</div>
                <div className="text-xs text-gray-300">KHÔNG → <strong>Present Simple</strong></div>
                <div className="font-mono text-[10px] text-blue-400">S + V(s/es)</div>
              </div>
            </div>

            <div className="space-y-4">
              <div className="font-mono text-[10px] text-gray-500 tracking-widest uppercase">QUÁ KHỨ → hỏi Q2b</div>
              <div className="bg-navy-900/50 border-l-4 border-l-red-500 p-4 rounded-r-xl space-y-2">
                <div className="text-xs font-bold text-red-400">Đã kết thúc hoàn toàn?</div>
                <div className="text-xs text-gray-300"><strong>Past Simple</strong></div>
                <div className="font-mono text-[10px] text-blue-400">S + V2/ed</div>
              </div>
              <div className="bg-navy-900/50 border-l-4 border-l-red-700 p-4 rounded-r-xl space-y-2">
                <div className="text-xs font-bold text-red-700">Đang dở giữa chừng?</div>
                <div className="text-xs text-gray-300"><strong>Past Continuous</strong></div>
                <div className="font-mono text-[10px] text-blue-400">S + was/were + V-ing</div>
              </div>
              <div className="bg-navy-900/50 border-l-4 border-l-purple-500 p-4 rounded-r-xl space-y-2">
                <div className="text-xs font-bold text-purple-400">Kết quả còn liên quan?</div>
                <div className="text-xs text-gray-300"><strong>Present Perfect</strong></div>
                <div className="font-mono text-[10px] text-blue-400">S + have/has + V3</div>
              </div>
            </div>

            <div className="space-y-4">
              <div className="font-mono text-[10px] text-gray-500 tracking-widest uppercase">TƯƠNG LAI → hỏi Q2c</div>
              <div className="bg-navy-900/50 border-l-4 border-l-blue-500 p-4 rounded-r-xl space-y-2">
                <div className="text-xs font-bold text-blue-400">Quyết định tức thì?</div>
                <div className="text-xs text-gray-300"><strong>Future Simple (will)</strong></div>
                <div className="font-mono text-[10px] text-blue-400">S + will + V</div>
              </div>
              <div className="bg-navy-900/50 border-l-4 border-l-blue-700 p-4 rounded-r-xl space-y-2">
                <div className="text-xs font-bold text-blue-700">Kế hoạch có từ trước?</div>
                <div className="text-xs text-gray-300"><strong>be going to</strong></div>
                <div className="font-mono text-[10px] text-blue-400">S + am/is/are + going to + V</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto rounded-2xl border border-white/10">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {[
                { v: 'Tôi ăn cơm.', s: 'thói quen', t: 'Present Simple', e: 'I eat rice. (mỗi ngày)', c: 'bg-green-500/10 text-green-400' },
                { v: 'Tôi đang ăn cơm.', s: 'đang xảy ra', t: 'Present Continuous', e: 'I am eating rice.', c: 'bg-blue-500/10 text-blue-400' },
                { v: 'Tôi đã ăn cơm. (hôm qua)', s: 'hoàn toàn kết thúc', t: 'Past Simple', e: 'I ate rice yesterday.', c: 'bg-red-500/10 text-red-400' },
                { v: 'Tôi đang ăn thì điện thoại reo.', s: 'đang dở (quá khứ)', t: 'Past Continuous', e: 'I was eating when it rang.', c: 'bg-orange-500/10 text-orange-400' },
                { v: 'Tôi vừa ăn xong rồi. (→ no rồi)', s: 'xong, còn ảnh hưởng', t: 'Present Perfect', e: 'I have just eaten.', c: 'bg-purple-500/10 text-purple-400' },
                { v: 'Tôi đã ăn trước khi cô ấy đến.', s: 'trước mốc quá khứ', t: 'Past Perfect', e: 'I had eaten before she came.', c: 'bg-indigo-500/10 text-indigo-400' },
              ].map((row, i) => (
                <tr key={i} className="hover:bg-white/5 transition-colors">
                  <td className="px-6 py-4 text-gray-400 italic">{row.v}</td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 font-mono text-[11px] text-blue-400">{row.t}</td>
                  <td className="px-6 py-4 font-bold font-mono">{row.e}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="bg-yellow-500/5 border border-yellow-500/20 p-6 rounded-2xl space-y-4">
          <div className="font-mono text-[10px] text-yellow-500 tracking-widest uppercase">// ĐIỂM KHÓ — PERFECT vs SIMPLE</div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-sm">
            <div className="space-y-2">
              <div className="font-bold text-red-400">Dùng Past Simple khi:</div>
              ...
              <div className="italic text-red-400/60">I met her in 2019.</div>
            </div>
            <div className="space-y-2">
              <div className="font-bold text-purple-400">Dùng Present Perfect khi:</div>
              <ul className="list-disc list-inside text-gray-400 space-y-1">
              ...
              <div className="italic text-purple-400/60">I have met her before. (→ quen rồi)</div>
            </div>
          </div>
        </div>

        <div className="bg-navy-950 p-8 rounded-[2rem] space-y-6">
          <div className="font-mono text-[10px] tracking-[0.3em] text-white/40 uppercase">// MẸO NHỚ TRẠNG THÁI HÀNH ĐỘNG</div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
            <div className="flex gap-4">
              <span className="text-2xl opacity-30 font-mono">●</span>
              <div>
                <div className="font-bold mb-1">-ing = đang xảy ra</div>
                <div className="text-gray-500 text-xs">Thấy "-ing" → hành động chưa kết thúc (Continuous)</div>
              </div>
            </div>
            <div className="flex gap-4">
              <span className="text-2xl opacity-30 font-mono">✓</span>
              <div>
                <div className="font-bold mb-1">V3 sau have/has/had = đã hoàn thành</div>
                <div className="text-gray-500 text-xs">have eaten = đã ăn xong / had eaten = đã ăn trước đó</div>
              </div>
            </div>
            <div className="flex gap-4">
              <span className="text-2xl opacity-30 font-mono">↩</span>
              <div>
                <div className="font-bold mb-1">Kết quả còn → Perfect</div>
                <div className="text-gray-500 text-xs">"Tôi vừa ăn" = no rồi → have eaten. "Tôi đã ăn hôm qua" → ate.</div>
              </div>
            </div>
            <div className="flex gap-4">
              <span className="text-2xl opacity-30 font-mono">②</span>
              <div>
                <div className="font-bold mb-1">2 hành động quá khứ → Past Perfect</div>
                <div className="text-gray-500 text-xs">Hành động nào xảy ra TRƯỚC → had + V3. Hành động sau → Past Simple.</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-4 border-b border-white/10 pb-4">
          <span className="bg-white text-navy-900 px-3 py-1 font-mono text-xs font-bold tracking-widest rounded">CÔNG CỤ</span>
          <h2 className="text-2xl font-bold">Trợ Động Từ — "Xương Sống" Của Tiếng Anh</h2>
        </div>
        <p className="text-sm text-gray-400">Trợ động từ là "công cụ hỗ trợ" cho động từ chính. Không có trợ động từ → câu tiếng Anh không thể hỏi, phủ định, hay diễn đạt thời gian.</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <div key={i} className="glass p-6 rounded-2xl space-y-4">
              ...
              <div className="font-mono text-[11px] text-blue-400">{card.m}</div>
              ...
              <div className="inline-block font-mono text-[11px] font-bold px-3 py-1 bg-white text-navy-900 rounded-full">{card.p}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-4 border-b border-white/10 pb-4">
          <span className="bg-white text-navy-900 px-3 py-1 font-mono text-xs font-bold tracking-widest rounded">TẦNG 04</span>
          <h2 className="text-2xl font-bold">Ma Trận Công Thức — Loại Câu × Thì</h2>
        </div>
        <div className="overflow-x-auto rounded-2xl border border-white/10">
          <table className="w-full text-center text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {[
                ...
                <tr key={i} className="hover:bg-white/5 transition-colors">
                  <td className="px-6 py-4 text-left font-bold bg-navy-950/30">{row.l}</td>
                  <td className="px-6 py-4 font-mono text-[11px] text-blue-400">{row.pr}</td>
                  <td className="px-6 py-4 font-mono text-[11px] text-blue-400">{row.pa}</td>
                  <td className="px-6 py-4 font-mono text-[11px] text-blue-400">{row.pp}</td>
                  <td className="px-6 py-4 font-mono text-[11px] text-blue-400">{row.f}</td>
                </tr>
              ))}
              <tr className="bg-red-500/5">
                <td colSpan={5} className="px-6 py-4 text-[11px] text-red-400 font-mono italic">
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-4 border-b border-white/10 pb-4">
          <span className="bg-white text-navy-900 px-3 py-1 font-mono text-xs font-bold tracking-widest rounded">LỖI PHỔ BIẾN</span>
          <h2 className="text-2xl font-bold">8 Lỗi Hay Mắc Khi Chuyển Câu Việt → Anh</h2>
        </div>
        <div className="grid gap-4">
          {[
            ...
            <div key={i} className="glass p-6 rounded-2xl border-l-4 border-l-red-500 space-y-2">
              <div className="text-red-400 font-mono text-sm">✗ {err.w}</div>
              <div className="text-green-400 font-mono text-sm">✓ {err.c}</div>
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-4 border-b border-white/10 pb-4">
          <span className="bg-white text-navy-900 px-3 py-1 font-mono text-xs font-bold tracking-widest rounded">MẸO BỔ SUNG</span>
          <h2 className="text-2xl font-bold">Mẹo Nói & Nhớ Tự Nhiên Hơn</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <div key={i} className="glass p-6 rounded-2xl space-y-3">
              ...
              <div className="font-bold text-sm">{tip.t}</div>
              <div className="text-xs text-gray-400 leading-relaxed">{tip.b}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-4 border-b border-white/10 pb-4">
          <span className="bg-white text-navy-900 px-3 py-1 font-mono text-xs font-bold tracking-widest rounded">CHECKLIST</span>
          <h2 className="text-2xl font-bold">Kiểm Tra Trước Khi Nói / Viết</h2>
        </div>
        <div className="glass p-8 rounded-[2rem] space-y-6">
          <div className="font-mono text-[10px] tracking-[0.3em] text-gray-500 uppercase">// 5 bước kiểm tra cuối cùng</div>
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="flex items-start gap-4 text-sm text-gray-300 border-b border-white/5 pb-4 last:border-0">
                <div className="w-5 h-5 border border-white/20 rounded-md flex-shrink-0 mt-0.5" />
                <span>{check}</span>
              </div>
            ))}
          </div>
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-4 border-b border-white/10 pb-4">
          <span className="bg-white text-navy-900 px-3 py-1 font-mono text-xs font-bold tracking-widest rounded">BỔ SUNG A</span>
          <h2 className="text-2xl font-bold">Động Từ Khiếm Khuyết — Phân Biệt Sắc Thái</h2>
        </div>
        <p className="text-sm text-gray-400 leading-relaxed">
        ...
        <div className="overflow-x-auto rounded-2xl border border-white/10">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {[
                { m: 'can', y: 'Khả năng hiện tại / xin phép', v: 'I can swim. / Can I go now?', l: 'Hiện tại/tương lai. Không chia -s, -ing.', c: 'text-green-400' },
                { m: 'could', y: 'Khả năng quá khứ / yêu cầu lịch sự', v: 'He could run fast. / Could you help me?', l: 'Lịch sự hơn can.', c: 'text-blue-400' },
                { m: 'be able to', y: 'Khả năng — mọi thì', v: 'I will be able to drive.', l: 'Thay thế can/could khi cần chia thì.', c: 'text-yellow-400' },
                { m: 'must', y: 'Bắt buộc (nội tâm / quy tắc)', v: 'You must submit the assignment.', l: '★★★ Bắt buộc cao', c: 'text-red-400' },
                { m: 'have to', y: 'Bắt buộc (hoàn cảnh)', v: 'I have to go, I\'m late.', l: '★★★ Bắt buộc cao', c: 'text-red-400' },
                { m: 'should', y: 'Lời khuyên / nên làm', v: 'You should sleep early.', l: '★★ Lời khuyên', c: 'text-green-400' },
                { m: 'may', y: 'Xin phép lịch sự / khả năng (50/50)', v: 'May I come in? / It may rain.', l: '★ Xin phép / ~50%', c: 'text-yellow-400' },
                { m: 'might', y: 'Khả năng thấp / suy đoán', v: 'He might not come.', l: '★ Không chắc ~30%', c: 'text-gray-400' },
              ].map((row, i) => (
                <tr key={i} className="hover:bg-white/5 transition-colors">
                  <td className={`px-6 py-4 font-bold font-mono ${row.c}`}>{row.m}</td>
                  <td className="px-6 py-4">{row.y}</td>
                  <td className="px-6 py-4 text-gray-400 italic">{row.v}</td>
                  <td className="px-6 py-4 text-[10px] font-mono">{row.l}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <div key={i} className={`glass p-6 rounded-2xl border-l-4 ${item.c} space-y-2`}>
              <div className="font-mono font-bold text-accent">{item.t}</div>
              <div className="text-xs text-gray-300">{item.b}</div>
              <div className="text-xs italic text-gray-500">{item.ex}</div>
            </div>
          ))}
        </div>

        <div className="glass p-8 rounded-[2rem] space-y-6">
          <div className="font-mono text-[10px] tracking-[0.2em] text-gray-500 uppercase">// MODAL + HAVE + V3 — PHÁN ĐOÁN / TIẾC NUỐI</div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              <div key={i} className="bg-navy-900/50 p-4 rounded-xl space-y-2">
                <div className="font-mono font-bold text-accent text-sm">{item.t}</div>
                <div className="text-xs text-gray-400">{item.b}</div>
                <div className="text-xs italic text-gray-500">{item.ex}</div>
              </div>
            ))}
          </div>
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-4 border-b border-white/10 pb-4">
          <span className="bg-white text-navy-900 px-3 py-1 font-mono text-xs font-bold tracking-widest rounded">BỔ SUNG B</span>
          <h2 className="text-2xl font-bold">Động Từ Trong Câu Tiếng Việt — Xác Định Thì</h2>
        </div>
        <p className="text-sm text-gray-400 leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <div key={i} className="glass p-6 rounded-2xl space-y-3 border-t-4 border-t-accent">
              <div className="font-mono text-[9px] text-gray-500 tracking-widest uppercase">{item.t}</div>
              <div className="font-bold text-sm">{item.n}</div>
              <div className="text-xs text-gray-400 whitespace-pre-line leading-relaxed">{item.d}</div>
              <div className="text-xs italic text-accent border-t border-white/5 pt-3">{item.ex}</div>
            </div>
          ))}
          <div className="glass p-6 rounded-2xl space-y-3 border-t-4 border-t-green-500 md:col-span-2">
            <div className="font-mono text-[9px] text-gray-500 tracking-widest uppercase">TRẠNG THÁI 5 — THÓI QUEN / SỰ THẬT</div>
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-4 border-b border-white/10 pb-4">
          <span className="bg-white text-navy-900 px-3 py-1 font-mono text-xs font-bold tracking-widest rounded">BỔ SUNG D</span>
          <h2 className="text-2xl font-bold">Stative Verbs — Động Từ Trạng Thái Không Dùng -ing</h2>
        </div>
        <p className="text-sm text-gray-400">Nhóm động từ này <strong>không bao giờ dùng -ing</strong> dù đang xảy ra tại thời điểm nói.</p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={i} className={`glass p-4 rounded-xl border-t-4 ${item.c} space-y-3`}>
              <div className="font-mono text-[9px] text-gray-500 tracking-widest uppercase">{item.t}</div>
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-4 border-b border-white/10 pb-4">
          <span className="bg-white text-navy-900 px-3 py-1 font-mono text-xs font-bold tracking-widest rounded">BỔ SUNG F</span>
          <h2 className="text-2xl font-bold">Câu Hỏi Lồng Nhau — Embedded Questions</h2>
        </div>
        <div className="bg-navy-950 p-10 rounded-[2.5rem] space-y-8">
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-4 border-b border-white/10 pb-4">
          <span className="bg-white text-navy-900 px-3 py-1 font-mono text-xs font-bold tracking-widest rounded">BỔ SUNG G</span>
          <h2 className="text-2xl font-bold">Mẹo Ngữ Pháp Cốt Lõi — Nhớ Nhóm, Dùng Đúng 80%</h2>
        </div>
        <div className="bg-navy-950 p-10 rounded-[2.5rem] grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
      ...
      {/* FOOTER */}
      <footer className="border-t border-white/10 pt-8 flex flex-col md:flex-row justify-between gap-4 text-[10px] font-mono text-gray-500 uppercase tracking-widest">
